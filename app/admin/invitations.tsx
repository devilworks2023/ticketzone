...
export default function InvitationsScreen() {
  ...
  const [newCodeType, setNewCodeType] = useState<'promoter' | 'seller'>('promoter');
  const [maxUses, setMaxUses] = useState('1');
  const [expiresInDays, setExpiresInDays] = useState('30');
  const [isCreating, setIsCreating] = useState(false);

  const codesQuery = trpc.auth.getInvitationCodes.useQuery();
  const createMutation = trpc.auth.createInvitationCode.useMutation();
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <View>
            ...
            <Text style={styles.subtitle}>
              Genera códigos para que promotores y vendedores se registren
            </Text>
          </View>
        ...
        <View style={styles.statsRow}>
          <View style={styles.statCard}>
            <Briefcase color={Colors.dark.primary} size={20} />
            <Text style={styles.statValue}>
              {codes.filter(c => c.type === 'promoter').length}
            </Text>
        ...
        {codesQuery.isLoading ? (
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySubtitle}>
              Crea códigos de invitación para permitir el registro de promotores y vendedores
            </Text>
          </View>
        ) : (
              ...
              <View key={code.id} style={[styles.codeCard, !code.isActive && styles.codeCardInactive]}>
                <View style={styles.codeHeader}>
                  <View style={styles.codeTypeContainer}>
                    {code.type === 'promoter' ? (
                      <Briefcase color={Colors.dark.primary} size={16} />
                    ) : (
                      <Users color={Colors.dark.secondary} size={16} />
                    )}
                    <Text style={styles.codeType}>
                      {code.type === 'promoter' ? 'Promotor' : 'Vendedor'}
                    </Text>
                  </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.typeSelector}>
              <TouchableOpacity
                style={[styles.typeOption, newCodeType === 'promoter' && styles.typeOptionSelected]}
                onPress={() => setNewCodeType('promoter')}
              >
                <Briefcase color={newCodeType === 'promoter' ? '#000' : Colors.dark.textMuted} size={20} />
                <Text style={[styles.typeOptionText, newCodeType === 'promoter' && styles.typeOptionTextSelected]}>
                  Promotor
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
